...
//                    .listRowSeparator(.hidden)
    ...
    private var settingsSection: some View {
        Section {
            SettingToggle(isOn: $noRepeat, icon: "repeat", foregroundColor: .white, backgroundColor: .accent, title: "不重复抽取", description: "已经被抽中的选项不会被抽中")

            SettingToggle(isOn: $equalWeight, icon: "equal.square", foregroundColor: .white, backgroundColor: .accent, title: "等概率抽取", description: "抽取时忽略选项的权重")

            SettingToggle(isOn: $enableSound, icon: "speaker.wave.3", foregroundColor: .white, backgroundColor: .accent, title: "声音", description: nil)
    ...
    private var rotationTimePicker: some View {
        HStack {
            SettingIconView(icon: .system(icon: "stopwatch", foregroundColor: .white, backgroundColor: .accent))
